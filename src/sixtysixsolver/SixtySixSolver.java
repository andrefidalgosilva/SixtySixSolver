...
public class SixtySixSolver {
	...
	private static ArrayList<Solution> perfectWholeSolutions = new ArrayList<Solution>();
	private static int numberOfSolutions = 0;
	private static int numberOfPerfectSolutions = 0;
	private static int numberOfPerfectWholeSolutions = 0;
	...
	public static void main(String[] args) {
		...
		if (args.length != 0) {
			...
			perfectWhole = (args[0].equals("-perfectwhole"));
			if (!args[0].equals("-onlyone") && !args[0].equals("-noparse")
					&& !args[0].equals("-random")
					&& !args[0].equals("-perfectsolutions")
					&& !args[0].equals("-perfectwhole")) {
				System.out
						.println("Invalid argument. Running with no arguments...");
				try {
					Thread.sleep(1200);
				} catch (InterruptedException ex) {
					Thread.currentThread().interrupt();
		...
		if (noParse) {
			...
			System.out.println("\nTime taken: " + (timeTaken / 1000000000)
					+ " seconds.");
			return;
		}
		...
		if (random && foundSolution && numberOfSolutions > 1) {
			...
			System.out.println("SOLUTION: \n" + randomSolution.getA()
					+ " + 13 * " + randomSolution.getB() + " / "
					+ randomSolution.getC() + " + " + randomSolution.getD()
					+ " + 12 * " + randomSolution.getE() + " - "
		...
		if (perfectSolution) {
			...
			for (Solution s : perfectSolutions) {
				System.out.println("PERFECT SOLUTION: \n" + s.getA()
						+ " + 13 * " + s.getB() + " / " + s.getC() + " + "
						+ s.getD() + " + 12 * " + s.getE() + " - " + s.getF()
						+ " - 11 + " + s.getG() + " * " + s.getH() + " / "
						+ s.getI() + " - 10 = 66\n");
			}
			long endTime = System.nanoTime();
			long timeTaken = (endTime - startTime);
			System.out.println("\nNumber of perfect solutions found: "
					+ numberOfPerfectSolutions);
			System.out.println("\nTime taken: " + (timeTaken / 1000000000)
					+ " seconds.");
			return;
		...
		if (perfectWhole) {
			System.out
					.println("\nFound solutions! "
							+ "Only whole perfect solutions requested (no digits repeated, "
							+ "only divisions with no remainders).");
			try {
				Thread.sleep(2000);
			} catch (InterruptedException ex) {
				Thread.currentThread().interrupt();
			...
			System.out.println("WHOLE PERFECT SOLUTIONS: \n");
			for (Solution s : perfectWholeSolutions) {
				System.out.println("WHOLE PERFECT SOLUTION: \n" + s.getA()
						+ " + 13 * " + s.getB() + " / " + s.getC() + " + "
						+ s.getD() + " + 12 * " + s.getE() + " - " + s.getF()
						+ " - 11 + " + s.getG() + " * " + s.getH() + " / "
						+ s.getI() + " - 10 = 66\n");
			...
			long endTime = System.nanoTime();
			long timeTaken = (endTime - startTime);
			System.out.println("\nNumber of whole perfect solutions found: "
					+ numberOfPerfectWholeSolutions);
			System.out.println("\nTime taken: " + (timeTaken / 1000000000)
					+ " seconds.");
			return;
		...
		if (foundSolution) {
			...
			for (Solution s : solutions) {
				System.out.println("SOLUTION: \n" + s.getA() + " + 13 * "
						+ s.getB() + " / " + s.getC() + " + " + s.getD()
						+ " + 12 * " + s.getE() + " - " + s.getF() + " - 11 + "
						+ s.getG() + " * " + s.getH() + " / " + s.getI()
						+ " - 10 = 66\n");
			}
			long endTime = System.nanoTime();
			long timeTaken = (endTime - startTime);
	...
	private static boolean equationEquals66(int a, int b, int c, int d, int e,
			int f, int g, int h, int i) {
		boolean result = false;

		try {
			result = ((a + 13 * (double) b / c + d + 12 * e - f - 11 + g
					* (double) h / i - 10) == 66);
		} catch (ArithmeticException ae) { // this is here in the unlikely event
											// of a zero division
			return false;
		}
	...
	private static boolean equationEqualsWhole66(int a, int b, int c, int d,
			int e, int f, int g, int h, int i) {
		boolean result = false;

		if ((13 * b % c != 0) || (g * h % i != 0))
			return false; //only whole divisions
	...
	private static boolean allDifferent(int a, int b, int c, int d, int e,
			int f, int g, int h, int i) {
		return new HashSet<Integer>(Arrays.asList(a, b, c, d, e, f, g, h, i))
				.size() == 9;
	}
}
